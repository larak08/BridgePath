...
function Profile() {
  ...
  const handleSave = async (e) => {
    ...
    // Clean up data: if they are a child, ensure no expertise data is sent to the DB
    const isChild = formData.ageGroup === 'Child';

    const profileUpdates = {
      first_name: formData.firstName,
      last_name: formData.lastName,
      age_group: formData.ageGroup,
      categories: isChild ? [] : formData.selectedCategories,
      specific_skills: isChild ? "" : formData.specificSkills, 
        ...
        {isSaved ? (
          <div style={{width: '100%', textAlign: 'center'}}>
            ...
            <div style={{textAlign: 'left', background: '#fcfaff', padding: '30px', borderRadius: '20px', marginTop: '20px'}}>
              ...
              <p>{formData.firstName} {formData.lastName} ({formData.ageGroup})</p>
              
              {/* Only show expertise in summary if NOT a child */}
              {formData.ageGroup !== 'Child' && (
                <>
                  <p style={styles.summaryLabel}>Specific Expertise</p>
                  <p>{formData.specificSkills || "No specific skills listed"}</p>
              ...
              <p style={styles.summaryLabel}>Bio</p>
              <p>{formData.description || "No bio provided"}</p>
            </div>
          ...
          <form style={{ width: '100%' }} onSubmit={handleSave}>
            ...
            {/* CHILD RESTRICTION: Hide Expertise if 'Child' is selected */}
            {formData.ageGroup !== 'Child' && (
              <>
            ...
            <textarea style={{...styles.input, height: '100px', resize: 'none'}} placeholder={formData.ageGroup === 'Child' ? "What do you want to learn?" : "A brief professional bio..."} value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} />
